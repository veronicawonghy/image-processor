(function(imageproc) {
    ...
    imageproc.comicColor = function(inputData, outputData, saturation) {
        ...
        for (var i = 0; i < inputData.data.length; i += 4) {
            ...
            var convertedHSV = imageproc.fromRGBToHSV(r,g,b);
            var h = convertedHSV["h"];
            var s = convertedHSV["s"];
            var v = convertedHSV["v"];
            // then, increase the saturation by the saturation factor
            // ***** beware of the final range of the saturation *****
            s = s*saturation > 1.0 ? 1.0 : s*saturation < 0.0 ? 0.0 : s*saturation;
            // after that, convert it back to RGB
            var convertedRGB = imageproc.fromHSVToRGB(h,s,v);
            var r = convertedRGB["r"];
            var g = convertedRGB["g"];
            var b = convertedRGB["b"];
            ...
            var minimumDistance = Math.hypot(palette[0][0]-r,palette[0][1]-g, palette[0][2]-b);
            var index = 0;
            for (var j = 1; j < palette.length; j++){
                var distance = Math.hypot(palette[j][0]-r,palette[j][1]-g, palette[j][2]-b)
